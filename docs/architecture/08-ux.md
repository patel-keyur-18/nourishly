...
**Content:** meal slot selector (**pre-selected by time of day and the user's own pattern**) · a large search field · segmented access to Recents / Favourites / Templates / My Foods · barcode button.
...
**Actions:** adjust serving/quantity · change meal slot · save · favourite · report a data error.
...
**Content:** month selector · monthly averages · a 30-day trend line with a 7-day moving average · goal-consistency percentages · chronically low and chronically high nutrients · previous-month comparison (gated per §25.6) · logging-consistency summary · score heat-map calendar (§9.2).
...
**Content:** account and sync · reminders · **data export** · **account and data deletion** · privacy policy · about, version, catalog version · disclaimers · accessibility options · a support/feedback path.
...
| Deep links resolve to a tab + stack, never a detached screen | Required for notification taps |
...
Declarative, typed routes (`go_router`, §12.2) so that notification deep links (`/insights/daily?date=…`) resolve to a tab plus a stack rather than a detached screen.

---

## 29. Notification Architecture

*Reminders ship in v1.0 (§9.1). Push infrastructure does not — see §29.3.*
...
- **Fully local.** No server, no push infrastructure, no data leaving the device (§29.3).

### 29.2 Reminder types

| Type | Trigger | Phase | Default |
|---|---|---|---|
| Water reminder | Time-based, or after N hours without a water log | v1.0 | Off |
| Meal logging reminder | User-set times per meal slot | v1.0 | Off |
| End-of-day summary | Configurable evening time, after rollover | v1.0 | **On** if any reminder is enabled — it is the one with clear value |
| Goal achieved | Water or protein target met | v1.0 | Off |
| Weekly report ready | Week-start day | v1.0 | Off |
| Re-engagement | N days without logging, **strictly capped and easy to disable** | v1.1 | Off — the one reminder type that can actively harm the user. Deferred deliberately: it should not ship before there is usage data to tune it against |
...
graph LR
    ...
    subgraph Adapters
        LOCAL["Local notifications<br/>v1.0"]
        PUSH["Server push<br/>v1.1, if ever"]
    end
    subgraph Platform
...
**Notifications are local-only in v1.0, deliberately, even though a backend exists.** Local scheduling works offline, needs no push tokens, requires no server infrastructure, and sends no data anywhere. Server push would only be justified by cross-device coordination or by content the device cannot compute — and the device computes everything, because all scoring and reporting is local (§25.8). Adding push because a backend happens to be available would be capability-driven design rather than requirement-driven.

`ReminderRule` is a syncable entity, so reminder preferences follow the user across devices with no push infrastructure at all.
